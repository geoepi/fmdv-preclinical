...
fluidRow(
  column(width = 12, align = "center",
         div(style = "max-width: 1000px; margin: 10px auto; text-align: left;",
             tags$p(tags$b("How this works:"),
                    "Each time you click ", tags$code("Run Experiment"),
                    " the simulation re-runs with a new random seed. ",
                    "The chosen seed is displayed below for reproducibility. ",
                    "After 1-2 minutes of simulation time, results will be plotted below."),
             tags$p(tags$b("Design summary:"),
                    "The App simulates 18 cattle in total. Two donors are inoculated with FMDV A24 Cruzeiro on study day 0. ",
                    "At 24 hours post-inoculation, donors are relocated to a room 4 naïve recipients (Group 1) for 24 h. ",
                    "Donors then move every 24 h to expose three additional groups of 4 cattle (Groups 2–4), each for 24 h. ",
                    "Contact-exposed animals are monitored up to 14 days post-exposure."),

             tags$p(tags$b("Model implementation:"),
                    "Simulation is executed with the ", tags$code("room-to-room"),
                    " module from the ",
                    tags$a(href = "https://github.com/geoepi/challengeABM",
                           target = "_blank", rel = "noopener noreferrer",
                           "challengeABM"), " R package.")
         )
  )
),
...
server <- function(input, output) {
  current_seed <- reactiveVal(NA_integer_)
