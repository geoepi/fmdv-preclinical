plot_exposure_linerange <- function(df, xlabel = "Exposure Type", ylabel = "Deviation from Mean") {
  ...
  p <- ggplot(data = df, aes(x = type, y = Q_0.5, color = sig)) +
    ...
    scale_color_manual(values = c('0' = 'gray50', '1' = 'darkorange')) +
    labs(title = " ",
         color = "Significant",
         x = xlabel,
         y = ylabel) +
